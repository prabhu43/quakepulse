...
async def list_earthquakes(
    ...
    try:
        ...
        logger.error(f"Database error in list_earthquakes: {exc}")
...
async def earthquakes_in_bbox(
    ...
    try:
        ...
        logger.error(f"Database error in earthquakes_in_bbox: {exc}")
        raise HTTPException(status_code=503, detail="Database temporarily unavailable")
    return quakes
...
async def get_earthquake(quake_id: str):
    ...
    try:
        ...
        logger.warning(f"Valkey cache miss/error for quake {quake_id}, falling back to DB")
    ...
    try:
        ...
        logger.error(f"Database error in get_earthquake: {exc}")
        raise HTTPException(status_code=503, detail="Database temporarily unavailable")
...
async def get_stats():
    ...
    try:
        ...
        logger.warning("Valkey cache miss/error for stats, falling back to DB")
    ...
    # Cache the stats (best-effort)
    try:
        await cache.set_cached_stats(stats.model_dump())
    except Exception:
        logger.warning("Failed to cache stats in Valkey")
